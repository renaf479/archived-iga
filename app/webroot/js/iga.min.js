...
//Timer
angular.module("timer",[]).directive("timer",["$compile",function(g){return{restrict:"E",replace:!1,scope:{interval:"=interval",startTimeAttr:"=startTime",countdownattr:"=countdown",autoStart:"&autoStart"},controller:["$scope","$element","$attrs",function(a,b,e){function c(){a.timeoutId&&clearTimeout(a.timeoutId)}function f(){a.seconds=("0"+Math.floor(a.millis/1E3%60)).slice(-2);a.minutes=("0"+Math.floor(a.millis/6E4%60)).slice(-2);a.hours=("0"+Math.floor(a.millis/36E5%24)).slice(-2);a.days=("0"+
Math.floor(a.millis/36E5/24)).slice(-2)}a.autoStart=e.autoStart||e.autostart;0===b.html().trim().length&&b.append(g("<span>{{millis}}</span>")(a));a.startTime=null;a.timeoutId=null;a.countdown=a.countdownattr&&0<=parseInt(a.countdownattr,10)?parseInt(a.countdownattr,10):void 0;a.isRunning=!1;a.$on("timer-start",function(){a.start()});a.$on("timer-resume",function(){a.resume()});a.$on("timer-stop",function(){a.stop()});a.start=b[0].start=function(){a.startTime=a.startTimeAttr?new Date(a.startTimeAttr):
new Date;a.countdown=a.countdownattr&&0<parseInt(a.countdownattr,10)?parseInt(a.countdownattr,10):void 0;c();d()};a.resume=b[0].resume=function(){c();a.countdownattr&&(a.countdown+=1);a.startTime=new Date-(a.stoppedTime-a.startTime);d()};a.stop=a.pause=b[0].stop=b[0].pause=function(){a.stoppedTime=new Date;c();a.$emit("timer-stopped",{millis:a.millis,seconds:a.seconds,minutes:a.minutes,hours:a.hours,days:a.days});a.timeoutId=null};b.bind("$destroy",function(){c()});a.millis=a.countdownattr?1E3*a.countdownattr:
0;f();var d=function(){a.millis=new Date-a.startTime;var b=a.millis%1E3;a.countdownattr&&(a.millis=1E3*a.countdown);f();if(0<a.countdown)a.countdown--;else if(0>=a.countdown){a.stop();return}a.timeoutId=setTimeout(function(){d();a.$digest()},a.interval-b);a.$emit("timer-tick",{timeoutId:a.timeoutId,millis:a.millis})};void 0!==a.autoStart&&!0!==a.autoStart||a.start()}]}}]);

//Transition
angular.module("ui.bootstrap.transition",[]).factory("$transition",["$q","$timeout","$rootScope",function(k,l,m){function h(a){for(var b in a)if(void 0!==n.style[b])return a[b]}var e=function(a,b,f){f=f||{};var d=k.defer(),c=e[f.animation?"animationEndEventName":"transitionEndEventName"],g=function(b){m.$apply(function(){a.unbind(c,g);d.resolve(a)})};c&&a.bind(c,g);l(function(){angular.isString(b)?a.addClass(b):angular.isFunction(b)?b(a):angular.isObject(b)&&a.css(b);c||d.resolve(a)});d.promise.cancel=
function(){c&&a.unbind(c,g);d.reject("Transition cancelled")};return d.promise},n=document.createElement("trans");e.transitionEndEventName=h({WebkitTransition:"webkitTransitionEnd",MozTransition:"transitionend",OTransition:"oTransitionEnd",transition:"transitionend"});e.animationEndEventName=h({WebkitTransition:"webkitAnimationEnd",MozTransition:"animationend",OTransition:"oAnimationEnd",transition:"animationend"});return e}]);

//Carousel
angular.module("ui.bootstrap.carousel",["ui.bootstrap.transition"]).controller("CarouselController",["$scope","$timeout","$transition","$q",function(a,e,n,l){function g(){function b(){h?(a.next(),g()):a.pause()}k&&e.cancel(k);var c=+a.interval;!isNaN(c)&&0<=c&&(k=e(b,c))}var c=this,d=c.slides=[],f=-1,k,h;c.currentSlide=null;c.select=function(b,m){function k(){c.currentSlide&&angular.isString(m)&&!a.noTransition&&b.$element?(b.$element.addClass(m),angular.forEach(d,function(b){angular.extend(b,{direction:"",
entering:!1,leaving:!1,active:!1})}),angular.extend(b,{direction:m,active:!0,entering:!0}),angular.extend(c.currentSlide||{},{direction:m,leaving:!0}),a.$currentTransition=n(b.$element,{}),function(b,c){a.$currentTransition.then(function(){h(b,c)},function(){h(b,c)})}(b,c.currentSlide)):h(b,c.currentSlide);c.currentSlide=b;f=l;g()}function h(b,c){angular.extend(b,{direction:"",active:!0,leaving:!1,entering:!1});angular.extend(c||{},{direction:"",active:!1,leaving:!1,entering:!1});a.$currentTransition=
null}var l=d.indexOf(b);void 0===m&&(m=l>f?"next":"prev");b&&b!==c.currentSlide&&(a.$currentTransition?(a.$currentTransition.cancel(),e(k)):k())};c.indexOfSlide=function(b){return d.indexOf(b)};a.next=function(){var b=(f+1)%d.length;if(!a.$currentTransition)return c.select(d[b],"next")};a.prev=function(){var b=0>f-1?d.length-1:f-1;if(!a.$currentTransition)return c.select(d[b],"prev")};a.select=function(b){c.select(b)};a.isActive=function(b){return c.currentSlide===b};a.slides=function(){return d};
a.$watch("interval",g);a.play=function(){h||(h=!0,g())};a.pause=function(){a.noPause||(h=!1,k&&e.cancel(k))};c.addSlide=function(b,e){b.$element=e;d.push(b);1===d.length||b.active?(c.select(d[d.length-1]),1==d.length&&a.play()):b.active=!1};c.removeSlide=function(b){var a=d.indexOf(b);d.splice(a,1);0<d.length&&b.active?a>=d.length?c.select(d[a-1]):c.select(d[a]):f>a&&f--}}]).directive("carousel",[function(){return{restrict:"EA",transclude:!0,replace:!0,controller:"CarouselController",require:"carousel",
template:'<div data-ng-mouseenter="pause()" data-ng-mouseleave="play()" class="carousel"><div class="carousel-inner" data-ng-transclude></div><a data-ng-click="prev()" class="carousel-control left transition" data-ng-show="slides().length > 1">&lsaquo;</a><a data-ng-click="next()" class="carousel-control right transition" data-ng-show="slides().length > 1">&rsaquo;</a></div>',scope:{interval:"=",noTransition:"=",noPause:"="}}}]).directive("slide",["$parse",function(a){return{require:"^carousel",restrict:"EA",
transclude:!0,replace:!0,template:"<div ng-class=\"{'active': leaving || (active && !entering),'prev': (next || active) && direction=='prev','next': (next || active) && direction=='next','right': direction=='prev','left': direction=='next'}\" class=\"item\" ng-transclude></div>",scope:{},link:function(e,n,l,g){if(l.active){var c=a(l.active),d=c.assign,f=e.active=c(e.$parent);e.$watch(function(){var a=c(e.$parent);a!==e.active&&(a!==f?f=e.active=a:d(e.$parent,a=f=e.active));return a})}g.addSlide(e,
n);e.$on("$destroy",function(){g.removeSlide(e)});e.$watch("active",function(a){a&&g.select(e)})}}}]);

//App
var igaApp = angular.module('igaApp', ['services', 'timer', 'ui.bootstrap.carousel']);
